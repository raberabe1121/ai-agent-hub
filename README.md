...
**主なターゲット：個人・小規模チームのAI利用者**

AWS AgentCoreなどのエンタープライズ向けAgentic AIが普及する中、個人や小規模チームが同等のガバナンス機能を**自己ホストで・無料で・AWSに依存せず**利用できることを目指しています。

- 「自分のAIエージェントが何をしたか追跡したい」→ **監査ログで全工程を可視化**
- 「スポンサー付きLLMの偏った回答を見抜きたい」→ **LLMバイアス検知で複数プロバイダーの回答を比較**
- 「AIの重要な判断を人間がコントロールしたい」→ **Human-in-the-loopで承認フローを強制**
- 「自分だけの知識ベースをAIに持たせたい」→ **RAGで知識ベースを構築・検索**
- 「機密情報がAIを経由して漏れないか心配」→ **X-Agent-Policyで配送レイヤーで遮断**

**二次ターゲット：エンタープライズのAI導入担当者・セキュリティ担当者**

AIを業務に導入する際の最大の障壁は「知能の欠如」ではなく**「ガバナンスの不在」**です。LangGraph/AutoGen/CrewAIと組み合わせて、エージェント間通信に監査性とガバナンスを追加したい場合。
...
distanceは正規化済みベクトル間のL2距離です（コサイン距離と等価）。値が小さいほど意味的に近いドキュメントです。取り得る値は0〜2の範囲で、実用上は1.0〜1.5付近に集中します。
...
`--max-distance 1.2`を基本として、ドメインに合わせて調整してください。
...
**LLMバイアス検知（Sponsored LLM対策）**

今後、スポンサー付きLLMが普及すると、ユーザーが気づかないまま特定の方向に誘導されるリスクがあります。AI Agent HubはLLMの出力を監査可能にすることで、この問題に対抗します。

```
同じクエリを複数のLLMプロバイダーに送信
    ↓
回答のばらつきをEntropy Monitorで測定
    ↓
一方向に偏った回答を検知してフラグを立てる
    ↓
Human-in-the-loopで人間が判断
```

**個人向けAgentic AI対応**

AWS AgentCore等のエンタープライズ向けAgentic AIが普及する中、個人・小規模チームが同等のガバナンス機能を自己ホストで利用できるようにします。

| AWS AgentCore | AI Agent Hub |
| --- | --- |
| Cognito・IAM・CloudTrail依存 | 自己ホスト可能 |
| エンタープライズ向け高コスト | OCI無料枠で動作 |
| Cedar言語によるポリシー | X-Agent-Policyヘッダー |
| Inbound/Outbound Auth | APIキー認証（実装予定） |
